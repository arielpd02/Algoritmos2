...
template<class T>
ColaPrior<T>::ColaPrior(const ColaPrior<T>& aCopiar): ColaPrior() {
    *this = aCopiar;
}

template<class T>
void ColaPrior<T>::copiar(typename ColaPrior<T>::Nodo* n) {
    if (n != nullptr) {
        copiar(n->izq);
        copiar(n->der);
        int freq = n->it->second.second;
...
template<class T>
ColaPrior<T>& ColaPrior<T>::operator=(const ColaPrior<T>& d) {
    destruir(raiz);
    mapping.clear();
    ultimo = 0;
    size = 0;
